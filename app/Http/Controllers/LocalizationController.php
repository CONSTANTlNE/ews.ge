...
class LocalizationController extends Controller
{
    ...
    public function createLanguage(Request $request)
    {
        ...
        foreach ($restaurants as $restaurant) {
            $restaurant->languages()->attach($lang->id);
        }
    ...
    public function deleteLanguage(Request $request)
    {
        ...
        foreach ($restaurants as $restaurant) {
            $restaurant->languages()->detach($language->id);
        }
    ...
    public function addTranslation(Request $request)
    {
        ...
        return view('admin.localization.static_translation', compact('locales', 'main', 'jsonData', 'keys', 'keyCount'));
    }
    ...
    public function autoTranslate(Request $request)
    {
        ...
        $from_lang = $request->from_file;
        $to_lang = $request->to_file;
        $filePath = base_path('lang/'.$from_lang.'.json');
        $filepath2 = base_path('lang/'.$to_lang.'.json');

        if (! file_exists($filePath) || ! file_exists($filepath2)) {
            abort(404, 'Localization file not found.');
        }

        // Read and decode the original translations
        $translations = json_decode(file_get_contents($filePath), true);

        $updatedTranslations = [];

        foreach ($translations as $key => $value) {
            $translated = new TranslationService()->translate([$to_lang], $value);
            $updatedTranslations[$key] = $translated['translations'][$to_lang];
        }

        // Encode back to pretty JSON
        $json = json_encode($updatedTranslations, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);

        // Save back to file (you can back up the original if needed)
        File::put($filepath2, $json);
